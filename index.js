...
async function setGroupRestriction(jid, sock, restrict, sender) {
    try {
        ...
        const adminIds = metadata.participants.filter(p => p.admin !== null).map(p => p.id);
        if (!adminIds.includes(sender)) {
            await sock.sendMessage(jid, { text: '⚠️ Kamu harus admin untuk melakukan ini.' });
            return;
        }
        await sock.groupSettingUpdate(jid, restrict ? 'announcement' : 'not_announcement');
    ...
    for (const [jid, schedules] of Object.entries(jadwal)) {
        for (const sch of schedules) {
            if (!sch.done) {
                ...
                const actionTime = now.set({ hour: hh, minute: mm, second: 0, millisecond: 0 });
                if (now >= actionTime) {
                    // Lakukan aksi buka/tutup grup
                    await setGroupRestriction(jid, sock, sch.action === 'close', sch.requester);
                    sch.done = true;
                    simpanJadwal();
                    console.log(`Jadwal ${sch.action} grup ${jid} dilakukan pada ${sch.time}`);
                }
            }
        }
    }
    ...
    sock.ev.on('messages.upsert', async (m) => {
        ...
        if (text.startsWith('!tagall')) {
            if (!from.endsWith('@g.us')) {
                await sock.sendMessage(from, { text: '⚠️ Perintah ini hanya untuk grup!' });
                return;
            }
        ...
        if (text.startsWith('!hidetag ')) {
            if (!from.endsWith('@g.us')) {
                await sock.sendMessage(from, { text: '⚠️ Perintah ini hanya bisa digunakan dalam grup.' });
                return;
            }
            const metadata = await sock.groupMetadata(from);
            const participants = metadata.participants;
        ...
        if (text.startsWith('!jadwal tambah ')) {
            ...
            if (args.length < 4) {
                await sock.sendMessage(from, { text: '⚠️ Format: !jadwal tambah HH:mm buka|tutup' });
                return;
            }
            const waktu = args[2];
            ...
            if (!/^([01]\d|2[0-3]):([0-5]\d)$/.test(waktu)) {
                await sock.sendMessage(from, { text: '⚠️ Format waktu harus HH:mm (24 jam).' });
                return;
            }
            if (aksi !== 'buka' && aksi !== 'tutup') {
                await sock.sendMessage(from, { text: '⚠️ Aksi harus "buka" atau "tutup".' });
                return;
            }
            ...
            simpanJadwal();
            await sock.sendMessage(from, { text: `✅ Jadwal ${aksi} grup pada pukul ${waktu} berhasil ditambahkan.` });
            return;
        }
        ...
        if (text === '!jadwal lihat') {
            ...
            if (!jadwal[from] || jadwal[from].length === 0) {
                await sock.sendMessage(from, { text: 'ℹ️ Belum ada jadwal untuk grup ini.' });
                return;
            }
            let daftar = '🗓️ Jadwal grup:\n\n';
            jadwal[from].forEach((j, i) => {
                daftar += `${i + 1}. Jam ${j.time} - ${j.action === 'close' ? 'Tutup' : 'Buka'} - ${
        ...
        if (text.startsWith('!jadwal hapus ')) {
            ...
            const args = pesan.split(' ');
            if (args.length < 3) {
                await sock.sendMessage(from, { text: '⚠️ Format: !jadwal hapus nomor' });
                return;
            }
